...
class UserCommonController extends AbstractController
{
    ...
    public function getUserInfos(Security $security)
    {
        ...
        if($security->isGranted('ROLE_ADMIN_ENTREPRISE') || $security->isGranted('ROLE_ADMIN_ORGANISME') || $security->isGranted('ROLE_ADMIN_DEPARTEMENT')){
            $admin = $user->getAdmin();
            $idEntite = $admin->getEntite()->getId();
            return $this->json(['data'=>$admin,'idEntite'=>$idEntite], 200, [], ['groups' => ['admin:read']]);
        }
        if ($security->isGranted('ROLE_SUPERADMIN') || $security->isGranted('ROLE_ADMIN') || $security->isGranted('ROLE_GESTIONNAIRE') || $security->isGranted('ROLE_USER')) {
            return $this->json($user, 200, [], ['groups' => ['user:read']]);
        }
    ...
    public function requestPassword(Request $request,\Swift_Mailer $mailer,Security $security, EntityManagerInterface $manager)
    {
        ...
        if($user){
            ...
            $message = (new \Swift_Message('AFRICLEAN SUPPORT REMOTE - Changement de mot de passe'))
                        ...
                        ->setTo($user->getEmail())         
                        ->setBody("<p>
                                Vous avez oublié votre mot de passe, veuillez entrez ce code pour confirmer la réinitialisation de votre mot de passe ${tokenPassword} <p>");
